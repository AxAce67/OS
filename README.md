...
*   **最小シェル**
    ...
    *   省力ショートカット: `Ctrl+V` で貼り付け（リアルタイムCOM1ブリッジ受信値を優先、無ければ `host.clip`、それも無ければ直近コマンド）
    *   シェル拡張: `set`, `alias`, `repeat`, `layout`, `inputstat`, `about`, `syscall`, `ring3`, `exec`, `procs`
    *   syscall検証: `syscall tick/version/write`（直呼び）と `syscall trap ...`（`int 0x80` 経由、`trap badptr` でEFAULT確認）
    *   環境変数syscall: `kGetEnv(5)` / `kSetEnv(6)` / `kUnsetEnv(7)`（`exec` 実行中の `envp` 参照、`syscall setenv/unsetenv` からも検証可）
    *   ring3準備確認: `ring3 stat`（user segment / TSS / user mapping / user code+stack 状態）
    *   ring3スタック準備: `ring3 prep` / `ring3 reset`
    *   ring3実行PoC: `ring3 run`（ring3コードから `int 0x80` で表示後、カーネルへ復帰）
    *   ring3異常系PoC: `ring3 runfault`（ring3側 bad pointer で `EFAULT` を確認）
    *   ring3外部バイナリ: `ring3 runfile <path>`（boot FS の `R3BIN01` 形式をロードして実行）
    *   実行コマンド: `exec [--unsetenv KEY ...] [--env-file PATH ...] [--env KEY=VALUE ...] <path> [args...]`（`ring3 runfile` の実行フロント。`argc/argv` と `envp` をユーザー側へ渡し、`--unsetenv` で削除、`--env-file` で一括読込、`--env` で上書き可能）
    *   `build.ps1` は `hello.r3bin` / `fault.r3bin` / `tick.r3bin` / `argc.r3bin` / `argv0head.r3bin` / `argv1head.r3bin` / `env0head.r3bin` / `env1head.r3bin` / `env2head.r3bin` / `getenvcwd0.r3bin` / `getenvlen.r3bin` / `getenvbad.r3bin` / `setenvok.r3bin` / `unsetenvbad.r3bin` / `setenvbadkey.r3bin` / `unsetenvbadkey.r3bin` を `disk/` に自動生成（`tools/generate_r3bin_samples.ps1`）
...
`disk/` を毎回クリーンにできない場合（ファイルロック等）は、既定では警告で続行します。  
再現性を優先して「クリーン失敗時に即エラー終了」したい場合は次を使用します。

```powershell
.\scripts\build.ps1 -StrictDisk
```

起動までの最小スモークテスト（`SYSTEM_READY` と `PROMPT_READY`）を自動確認する場合:
...
6.  起動時にWindowsクリップボードのASCIIスナップショットを `disk/host.clip` として取り込みます（`Ctrl+V` のフォールバック）。
7.  通常起動時は `tools/clip_bridge.ps1` が自動起動し、QEMUのCOM1(`tcp:127.0.0.1:4545`)へクリップボード更新を送信します（数秒ごと再送あり、UARTは9600bps）。
...
2. 起動時に `BOOT ERROR [Bxxx]` / `KERNEL PANIC [Kxxx]` が出る場合は、  
   エラー一覧を確認して原因層を特定する  
   - [Error Codes](c:/os/docs/error-codes.md)
   - [Ring3 Binary Format](c:/os/docs/ring3-binary-format.md)
3. `Bxxx` はブートローダー層、`Kxxx` はカーネル層として切り分ける
...
- 実行タイミング:
  ...
  - `schedule`（毎週: 日曜 03:00 UTC）
- 実行内容:
  - `.\scripts\build.ps1 -NoRun -StrictDisk`
  - `.\scripts\build.ps1 -Smoke -StrictDisk`
...
- `exec --env CWD=/tmp --env CWD=abc /getenvcwd0.r3bin` で後勝ち上書きが効く（`exec.ret=97`）
- `exec --unsetenv CWD --env CWD=abc /getenvcwd0.r3bin` のように削除と追加を併用できる（後段指定が最終状態）
- `exec --env-file /env.txt /getenvcwd0.r3bin` で `KEY=VALUE` 形式ファイルを読み込みできる（`#` 行と空行は無視）
