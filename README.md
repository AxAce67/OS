...
*   **最小シェル**
    ...
    *   実行コマンド: `exec <path> [args...]`（`ring3 runfile` の実行フロント。引数はユーザー側へ渡される）
    *   `build.ps1` は `hello.r3bin` / `fault.r3bin` / `tick.r3bin` / `argc.r3bin` / `argv0head.r3bin` / `argv1head.r3bin` を `disk/` に自動生成（`tools/generate_r3bin_samples.ps1`）
