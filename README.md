...
*   **最小シェル**
    ...
    *   環境変数syscall: `kGetEnv(5)` / `kSetEnv(6)` / `kUnsetEnv(7)`（`exec` 実行中の `envp` 参照）
    ...
    *   `build.ps1` は `hello.r3bin` / `fault.r3bin` / `tick.r3bin` / `argc.r3bin` / `argv0head.r3bin` / `argv1head.r3bin` / `env0head.r3bin` / `env1head.r3bin` / `env2head.r3bin` / `getenvcwd0.r3bin` / `getenvlen.r3bin` / `getenvbad.r3bin` / `setenvok.r3bin` / `unsetenvbad.r3bin` を `disk/` に自動生成（`tools/generate_r3bin_samples.ps1`）
