...
constexpr uint32_t kXhciHidAutoStartFailSetIdle = 7;
constexpr uint32_t kXhciHidAutoStartFailFindEndpoint = 8;
constexpr uint32_t kXhciHidAutoStartFailConfigEndpoint = 9;
...
const char* XhciHidAutoStartFailReasonName(uint32_t reason) {
    switch (reason) {
        ...
        case kXhciHidAutoStartFailSetConfiguration:
            return "set_config";
        case kXhciHidAutoStartFailSetIdle:
            return "set_idle";
        case kXhciHidAutoStartFailFindEndpoint:
            return "find_ep";
...
bool ExecuteHelpCommand() {
    ...
    console->PrintLine("help: cfg   repeat layout ime(on/off/toggle/stat/save/import/export/resetlearn) set alias syscall ring3 xhciinfo xhciregs xhcistop xhcistart xhcireset xhciinit xhcienableslot xhciaddress xhciconfigep xhciintrin xhcidesc xhcihidpoll xhcihidstat xhciauto xhciautostart mouseabs usbports");
    return true;
}
...
bool ExecuteInputDiagCommand() {
    ...
    console->Print(" cfg=");
    console->PrintDec(static_cast<int64_t>(g_xhci_hid_auto_config_value));
    console->Print(" ep=0x");
    console->PrintHex(g_xhci_hid_auto_endpoint_address, 2);
    console->Print(" dci=");
    console->PrintDec(static_cast<int64_t>(g_xhci_hid_auto_endpoint_id));
    console->Print(" len=");
